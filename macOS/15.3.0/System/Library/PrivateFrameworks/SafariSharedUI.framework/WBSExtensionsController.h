@class NSMutableDictionary, WKUserContentController, NSObject, NSDictionary, WBProfile, NSMutableArray, WBSCoreAnalyticsExtensionsStatistics, NSString, NSSet, WKWebsiteDataStore, NSTimer, NSMutableSet, NSPointerArray, NSArray, WBSExtensionsAnalyticsEventCoalescer, WBSDiagnosticStateCollector;
@protocol OS_dispatch_queue, WBSExtensionsControllerDelegate;

@interface WBSExtensionsController : NSObject <WBSExtensionsExportDataSource, WBSExtensionResourceVerifier, WBSCloudExtensionStateProvider> {
...
@property (class, readonly, nonatomic) BOOL safari_extensionsEnabled;
@property (class, readonly) double temporarySiteAccessTimeInterval;

@property (readonly, copy, nonatomic) NSString *crashReporterMessage;
@property (nonatomic) BOOL allowUnsignedExtensions;
...
+ (id)extensionURLToLoadFromPersistentStateURL:(id)a0 inBrowserViewController:(id)a1;
+ (id)persistentStateURLForExtensionURL:(id)a0 inBrowserViewController:(id)a1;
...
- (BOOL)_allowAppExtensionsWithoutDeveloperCertificates;
- (void)_cancelTimeoutForUnsignedExtensionsSetting;
- (void)_clearCachedCodeSigningInformationForExtensions;
- (id)_extensionURLToLoadFromPersistentStateURL:(id)a0;
- (void)_markExtensionAsReplaced:(id)a0;
- (id)_persistentStateURLForExtensionURL:(id)a0;
- (void)_replaceAppExtensionsWithWebExtension:(id)a0 developerIdentifier:(id)a1;
- (void)_setTimeoutForUnsignedExtensionsSetting;
- (void)_unsignedExtensionsSettingDidTimeout;
- (void)canToggleAllowUnsignedExtensionsAuthenticatingInWindow:(id)a0 completionHandler:(id /* block */)a1;
- (void)extensionBlocklistDidChange;
- (void)retryExtensionValidation:(id)a0;
- (id)safari_computeCodeSigningDictionaryOnDiskIfNecessaryForExtension:(id)a0 attemptRetryOnFailure:(BOOL)a1;
- (void)safari_disableUnsignedExtensionsIfNecessary;
- (BOOL)safari_isExtensionBlockedByBlocklist:(id)a0 developerIdentifier:(id)a1;
- (void)safari_replaceExtensionsObsoletedByExtension:(id)a0 developerIdentifier:(id)a1;
- (void)safari_showNewExtensionBannerIfNecessaryForExtensions:(id)a0;
- (void)safari_updateExtensionsCrashReporterMessage;
- (void)safari_validateContainingAppOfExtensionIfNecessary:(id)a0 attemptRetryOnFailure:(BOOL)a1;
- (void)dealloc;
- (id)init;
- (void).cxx_destruct;
