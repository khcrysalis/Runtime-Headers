@class NSNumber, CNUserDefaults, NSDistributedNotificationCenter, NSNotificationCenter, NSString, NSLocale, CRRecentContactsLibrary;
@protocol CNCDIOSLegacyIdentifierRegistration, CNFeatureFlags, CNNicknameProvider, CNKeyboardStateMonitor, CNAuthorizationContext, CNLocalizationServices, CNEntitlementVerification, CNWatchdog, CNOpenDirectoryFacade, CNTimeProvider, CNFileManager, CNProbabilityUtility, CNSchedulerProvider, CNMetricsReporter, _TtP18ContactsFoundation24CNKeychainFacadeProtocol_;

@interface CNEnvironmentTestDouble : CNEnvironment {
    NSNumber *_isAquaSessionStorage;
}

@property (retain) id<CNCDIOSLegacyIdentifierRegistration> legacyIdentifierRegistrar;
@property (retain) id<CNOpenDirectoryFacade> openDirectory;
@property (retain) id<CNKeyboardStateMonitor> keyboardStateMonitor;
@property (retain) id<CNNicknameProvider> nicknameProvider;
@property (retain) CRRecentContactsLibrary *coreRecentsLibrary;
@property (retain) id<CNMetricsReporter> metricsReporter;
...
- (void)setLegacyIdentifierRegistrar:(id)a0;
- (void)setOpenDirectory:(id)a0;
- (void)setKeyboardStateMonitor:(id)a0;
- (void)setNicknameProvider:(id)a0;
- (void)setCoreRecentsLibrary:(id)a0;
- (void)setMetricsReporter:(id)a0;
- (id)init;
- (void).cxx_destruct;
